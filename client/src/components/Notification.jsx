      ...
      <div className='flex-1 p-4'>
        <div className='flex items-start'>
          {message?.from_user_id?.profile_picture ? (
            ...
            <div className='h-10 w-10 rounded-full flex-shrink-0 mt-0.5 bg-gray-200 text-gray-600 grid place-items-center text-sm font-medium'>
              {(message?.from_user_id?.full_name?.[0] || 'U').toUpperCase()}
            </div>
          )}
          <div className='ml-3 flex-1'>
            <p className="text-sm font-medium text-gray-900">
              {message?.from_user_id?.full_name || 'Unknown User'} </p>
