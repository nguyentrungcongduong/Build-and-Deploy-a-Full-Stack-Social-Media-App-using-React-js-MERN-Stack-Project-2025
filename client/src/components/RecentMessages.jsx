...
//       });
//       if (data.success) {
//         //Group messages by sender and get the lastest message for each sender
//         const groupedMessages = data.messages.reduce((acc, message) => {
//           if (
//             !acc[senderId] ||
//             new Date(message.createAt) > new Date(acc[senderId].createAt)
//           ) {
  ...
  const fetchRecentMessages = async () => {
    try {
      ...
      if (data.success) {
        // Group messages by sender and get the lastest message for each sender
        const groupedMessages = data.messages.reduce((acc, message) => {
          const senderId = message.from_user_id._id; // ✅ fix lỗi senderId
          if (
            !acc[senderId] ||
            new Date(message.createdAt) > new Date(acc[senderId].createdAt) // ✅ fix key created_at
          ) {
            acc[senderId] = message;
          }
          return acc;
